...
model is backed up first - any run can always be undone.

Base architecture is YOLO26 (BASE_MODEL). Warm starting only works
WITHIN one architecture, so switching the base means a 'fresh' run:
the weights of a yolov8n cube model cannot be loaded into a YOLO26.

Usage: python train_cubes.py [epochs] [fresh]
       fresh = ignore the existing model and start from BASE_MODEL
"""
import os
import shutil
import sys
...
def main():
    ...
    # keep the working model whatever we are about to do: if the new one
    # turns out worse, this is the only way back. a 'fresh' run overwrites
    # cube_model.pt just as an additive one does, so it needs this too.
    ...
    if FRESH or not os.path.exists(MODEL_OUT):
        base = BASE_MODEL
        print("Training FROM SCRATCH (%s) on %d train / %d val images."
              % (BASE_MODEL, n_train, n_val))
        if FRESH:
